...
class GeometryOptimalTransport(nn.Module):
    ...
    def forward(self, source_feats, source_locs, target_locs, source_valid_mask=None, target_valid_mask=None):
        ...
        with torch.no_grad():
            # =========================================================
            # 🌟 核心改进：分块计算距离与 Mask，对抗显存碎片
            # =========================================================
            dist_sq = torch.empty((B, M, N), device=device, dtype=torch.float32)
            valid_connection = torch.empty((B, M, N), device=device, dtype=torch.bool)
            
            chunk_step = 512 
            for i in range(0, M, chunk_step):
                end = min(i + chunk_step, M)
                
                # 1. 计算局部距离: [B, chunk_M, N]
                d_chunk = torch.cdist(target_locs[:, i:end, :], source_locs, p=2.0) ** 2
                dist_sq[:, i:end, :] = d_chunk
                
                # 2. 局部计算有效连接
                v_chunk = d_chunk < (self.dist_thresh ** 2)
                
                # 修正 Source Mask 的应用: source_valid_mask 是 [B, N]
                # N 维度对应 d_chunk 的最后一维，所以用 unsqueeze(1) 广播
                if source_valid_mask is not None:
                    v_chunk &= source_valid_mask.unsqueeze(1)
                
                # 🌟 修正 Target Mask 的应用: target_valid_mask 是 [B, M]
                # M 维度对应 d_chunk 的第二维，所以取 [:, i:end] 后用 unsqueeze(2) 广播
                if target_valid_mask is not None:
                    # 注意这里只取两个维度 [B, i:end]
                    v_chunk &= target_valid_mask[:, i:end].unsqueeze(2)
                
                valid_connection[:, i:end, :] = v_chunk
                del d_chunk, v_chunk

            # 2. 原地将距离转换为 Log-Kernel
            # 直接在 dist_sq 的内存上操作，不创建新张量
            dist_sq.div_(-(self.epsilon + 1e-8))
            dist_sq.masked_fill_(~valid_connection, -1e9)
            log_K = dist_sq # 仅仅是重命名，指向同一块 8GB 内存
            ...
            for _ in range(self.num_iters):
                # 步骤 A: 原地加势能，减少临时 tensor
                log_K.add_(v)
                u = -torch.logsumexp(log_K, dim=2, keepdim=True)
                log_K.sub_(v) # 减回去复原
                
                # 步骤 B: 同样原地操作
                log_K.add_(u)
                v = -torch.logsumexp(log_K, dim=1, keepdim=True)
                log_K.sub_(u)
            ...
            # del log_K, u, v # 释放内存
            # 4. 计算最终 Attention
            # 这里是最后一个显存高峰，log_K + u + v 会产生一个 8GB 张量
            # 随后 exp 会产生另一个 8GB 张量。
            # 优化：原地操作
            log_K.add_(u)
            log_K.add_(v)
            attn = torch.exp_(log_K) # 原地 exp，不创建新张量
        ...
        return target_feats # , attn # 如果需要复用

# =========================================================================
# Main Model
# =========================================================================

class BiggerGait__SAM3DBody__Projection_Mask_Part_3D_Fill_Gaitbase_Share(BaseModel):
    ...
    def warp_features_with_ot(self, human_feat, mask_src, 
                              ...
                              H_feat, W_feat, target_H, target_W, s_chunk):
        """
        ...
        # =========================================================
        # 🌟 显存优化：时序特征池化 (使用 expand 替代 repeat_interleave)
        # =========================================================
        # 将 [n_seq, s_chunk * HW, C] 扩展为 [n_seq, s_chunk, s_chunk * HW, C]
        # 这里的 expand 是零显存开销的视图操作
        
        # 处理特征 [B, HW, C]
        flat_human_feat = flat_human_feat.reshape(n_seq, 1, s_chunk * H * W, C).expand(-1, s_chunk, -1, -1).reshape(B, -1, C)
        
        # 处理 3D 坐标 [B, HW, 3]
        flat_src_verts = flat_src_verts.reshape(n_seq, 1, s_chunk * H * W, 3).expand(-1, s_chunk, -1, -1).reshape(B, -1, 3)
        
        # 处理 Mask [B, HW]
        flat_src_mask = flat_src_mask.reshape(n_seq, 1, s_chunk * H * W).expand(-1, s_chunk, -1).reshape(B, -1)
        ...
        # 【注意】这里的 flat_src_verts 已经是扩展后的序列点云，
        # 但 midhip, R_comp, cam_t_tgt 依然是当前帧的，所以投影结果是“时序点云在当前帧的落点”
        ...
        if self.enable_visual:
            ...
            flat_color = flat_color.reshape(n_seq, s_chunk * H * W, 3).repeat_interleave(s_chunk, dim=0)
    ...
    def warp_features_with_ot_tpose(self, human_feat, mask_src, 
        ...
        if self.training and self.enable_visual:
            ...
            if not hasattr(self, '_debug_ply_saved'):
                self._debug_dump_apose_ply(src_idx_map[:s], t_pose_verts[:1], save_path="debug_apose_chunk0.ply")
    ...
    def forward(self, inputs):
        ...
        rgb_chunks = torch.split(rgb, CHUNK_SIZE, dim=1)
        ...
        for _, rgb_img in enumerate(rgb_chunks):
            ...
            with torch.no_grad():
                ...
                # --- 遮挡去重逻辑 (Z-Buffer) ---
                # 直接用全体顶点进行唯一一次投影，获取全局 Depth Map
                _, global_depth_map = self.get_source_vertex_index_map(
                    pred_verts, pred_cam_t, cam_int_src, 
                    h_feat, w_feat, target_h, target_w
                )

                # 只要深度小于 1e5 (有效投影点)，即视为前景
                generated_mask = (global_depth_map < 1e5).float() # [B, 1, 32, 16]
            ...
            # 显式删除不再需要的列表引用
            del processed_feat_list, features_to_use
            ...
            for b_cfg in self.branch_configs:
                ...
                if b_cfg.get('use_tpose', False):
                    ...
                    warp_feat, tgt_mask, tgt_color_flow = self.warp_features_with_ot(
                        ...
                        pred_verts, pred_keypoints, pred_cam_t, global_rot,
                        cam_int_src, cam_int_tgt, cam_t_tgt,
                        self.sils_size*2, self.sils_size, target_h, target_w, s_chunk=s
                    )
