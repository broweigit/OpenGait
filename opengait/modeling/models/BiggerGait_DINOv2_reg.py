...
class BiggerGait__DINOv2_Reg(BaseModel):
    ...
    def forward(self, inputs):
        ...
        for _, rgb_img in enumerate(rgb_chunks):
            with torch.no_grad():
                ...
                # --- [ TODO 0: 原始代码 (Original Code) ] ---
                # (当您运行 "with-register" 模型时，这会崩溃)
                ...
                # --- [ TODO 1: 排除 Registers (Exclude Registers) ] ---
                # (确保这个块被激活，以使代码首先运行起来)
                num_registers = 4 # default
                token_offset = 1 + num_registers # (1 CLS + 4 Reg = 5)

                intermediates = partial(nn.LayerNorm, eps=1e-6)(self.f4_dim*len(outs), elementwise_affine=False)(torch.concat(outs, dim=-1))[:,token_offset:]
                intermediates = rearrange(intermediates.view(n, s, self.image_size//7, self.image_size//14, -1), 'n s h w c -> (n s) c h w').contiguous()
                intermediates = list(torch.chunk(intermediates, self.total_layer_num, dim=1))

                human_mask = partial(nn.LayerNorm, eps=1e-6)(self.f4_dim, elementwise_affine=False)(outs[-1])[:,token_offset:].contiguous()
