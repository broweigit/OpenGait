...
class BiggerGait__SAM3DBody__Foundation_Gaitbase_Share(BaseModel):
    def build_network(self, model_cfg):
        ...
        self.num_parts = model_cfg["num_parts"]
    ...
    def forward(self, inputs):
        ...
        for _, rgb_img in enumerate(rgb_chunks):
            ...
            parts_fpn_feats = [[] for _ in range(self.num_parts)]
            ...
            for i in range(self.num_FPN):
                ...
                # sub_app_parts: Tuple of 8 tensors, each [B, 5120, H/8, W]
                sub_app_parts = torch.chunk(sub_app, self.num_parts, dim=2)

                # 3. 独立通过 HumanSpace_Conv
                for p_idx in range(self.num_parts):
                    # 找到对应的 Conv 模块索引
                    conv_idx = i * self.num_parts + p_idx
                    
                    # [B, 5120, H/8, W] ->[B,16(num_unknown), target_h/8, target_w]
                    reduced_part = self.HumanSpace_Conv[conv_idx](sub_app_parts[p_idx])
                    
                    parts_fpn_feats[p_idx].append(reduced_part)

            parts_input_feats = [torch.concat(f_list, dim=1) for f_list in parts_fpn_feats] # 8(parts) * [B, 16*num_FPN, h/8, w]

            current_chunk_parts_outs = [] # List of 8 tensors

            for p_idx in range(self.num_parts):
                part_feat = parts_input_feats[p_idx] # [B, C, H_p, W]
                
                # Reshape for GaitNet [n, c, s, h_p, w]
                part_feat = rearrange(part_feat.view(n, s, -1, part_feat.shape[2], part_feat.shape[3]), 
                                      'n s c h w -> n c s h w').contiguous()
                
                # 独立的 GaitNet Forward
                # out: [n, c_out, s, h_p, w] (GaitNet test_1 output)
                out = self.Gait_Nets[p_idx].test_1(part_feat)
                current_chunk_parts_outs.append(out)

            all_outs.append(current_chunk_parts_outs)

        final_embeds = [] # 用于存储 8 个 Part 的 Embedding
        final_logits = [] # 用于存储 8 个 Part 的 Logits
        ...
        if self.training:
            retval = {
                'training_feat': {
                    ...
                    'softmax': {'logits': torch.concat(logits_list, dim=-1), 'labels': labs},
                },
