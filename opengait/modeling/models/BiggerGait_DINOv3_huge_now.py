...
class BiggerGait__DINOv3_Huge_Now(BaseModel):
    def build_network(self, model_cfg):
        ...
        # ================== [修改 1] 升级下游网络 ==================
        # 原代码: self.Gait_Net = Baseline_Share(model_cfg)
        # 新代码: 使用带 Time 后缀的版本，配合 Config 中的 BasicBlock_Time
    ...
    def forward(self, inputs):
        ...
        for _, rgb_img in enumerate(rgb_chunks):
            with torch.no_grad():
                ...
                # ================== [修改 2] 分组归一化 (Grouped LayerNorm) ==================
                # 这里彻底替换了原有的 “Concat All -> Global LN -> Split” 逻辑
                
                target_tokens = h_feat * w_feat
                processed_groups = []

                # 按组遍历 (每次处理 self.group_layer_num 层，例如 2 层)
                for i in range(0, self.total_layer_num, self.group_layer_num):
                    # 1. 取出当前组的 Token 特征 (例如 2 层)
                    # list of [B, N_tokens, 1280]
                    group_raw = outs[i : i+self.group_layer_num]
                    
                    # 2. 拼接 -> [B, N_tokens, 2560]
                    group_feat = torch.concat(group_raw, dim=-1)
                    
                    # 3. 局部 LayerNorm (只对这 2 层做归一化)
                    curr_dim = self.f4_dim * self.group_layer_num
                    group_feat = partial(nn.LayerNorm, eps=1e-6)(curr_dim, elementwise_affine=False)(group_feat)
                    
                    # 4. 只取 Spatial Tokens (丢弃 CLS 等)
                    group_feat = group_feat[:, -target_tokens:]
                    
                    # 5. Reshape 为图像格式 -> [(n s), C, H, W]
                    group_feat = rearrange(
                        group_feat.view(n, s, h_feat, w_feat, -1), 
                        'n s h w c -> (n s) c h w'
                    ).contiguous()
                    
                    # 6. 直接喂给对应的 FPN 卷积层
                    # i 是 0, 2, 4...; fpn_idx 是 0, 1, 2...
                    fpn_idx = i // self.group_layer_num
                    group_feat = self.HumanSpace_Conv[fpn_idx](group_feat)
                    
                    processed_groups.append(group_feat)
                
                # 7. 拼接所有 FPN 输出
                intermediates = torch.concat(processed_groups, dim=1)
                # ===========================================================================
                ...
                human_mask = torch.ones(
                    (n * s, 1, h_feat, w_feat),
                    dtype=intermediates.dtype,
                    device=intermediates.device
                )
